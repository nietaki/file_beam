defmodule FileBeam.Middleware.ResponseCompression do
  ...
  defmodule State do
    ...
    def compress(%__MODULE__{} = state, %Response{body: whole_body} = response) when is_list(whole_body) do
      response = %Response{response | body: IO.iodata_to_binary(whole_body)}
      compress(state, response)
    end

    def compress(%__MODULE__{compression_type: type} = state, %Response{body: whole_body} = response) when is_binary(whole_body) do
    ...
    def compress(state, %Data{data: binary_data}) do
      ...
      compressed = :zlib.deflate(state.zstream, binary_data, :sync)
      {state, [%Data{data: compressed}]}
    end

    def compress(state, %Tail{} = tail) do
      last_compressed = :zlib.deflate(state.zstream, "", :finish)
